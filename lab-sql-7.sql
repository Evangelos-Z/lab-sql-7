...
# 1. In the table actor, which are the actors whose last names are not repeated? For example if you would sort the data in the table actor by last_name, you would see
...
select last_name, count(last_name) over (order by last_name) as frequency from actor
having count(last_name) = 1;
